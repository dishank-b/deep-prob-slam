...
for run_id in range(runs):
    ...
    uncalib_ODOMETRY_NOISE = gtsam.noiseModel.Diagonal.Sigmas(np.array([0.05, 0.05, 1*2*np.pi/180.0]))
    uncalib_MEASUREMENT_NOISE = gtsam.noiseModel.Diagonal.Sigmas(np.array([1*2*np.pi/180.0, 0.05])) # bearing (angle) noise, distance noise.
    ...
    running_metrics["robo_rmse"].append(rmse_robo)
    running_metrics["robo_maha"].append(mahalanobis_robo)
    running_metrics["land_rmse"].append(rmse_landmarks)
    running_metrics["land_maha"].append(mahalanobis_landmark)

    plt.clf()

for key in running_metrics.keys():
    print("{}: {}".format(key, np.array(running_metrics[key]).mean()))
