...
def report_on_progress(graph: gtsam.ISAM2,
                       current_estimate: gtsam.Values,
                       pose_each: int = 10,
                       pose_cov: bool = False,
                       quadric_cov: bool = False):
    ...
    for j in range(len(current_estimate.keys())):
        ...
        if chr(gtsam.symbolChr(key)) == 'l':
            ...
            # Plot convariance matrix of quadrics or plain frame
            if quadric_cov:
                gtsam_plot.plot_pose3(0, quadric.pose(), 0.2,
