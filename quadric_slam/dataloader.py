...
def tum_raw(dir, intrinsics):
    ...
    for file_name in file_names:
        ...
        instance.pose = gtsam.Pose3(gtsam.Rot3.Quaternion(instance.pose[-1], *instance.pose[3:-1]),
                                    gtsam.Point3(*instance.pose[:3]))
...
def tum_orb(path, intrinsics):
    ...
    orb_file = open(os.path.join(path, "CameraTrajectory_ORBVO.txt"), 'r')
    data = torch.load(os.path.join(path, "tum.pth"), map_location=torch.device('cpu'))
    ...
    for line in orb_file.readlines():
        ...
        if str_name in file_keys:
            key = str_name + ".png"
            orb_pose = gtsam.Pose3(gtsam.Rot3.Quaternion(split_line[-1], *split_line[4:-1]),
                                   gtsam.Point3(*split_line[1:4]))
            instance = Instance(bbox=data['predicted_boxes'][key].numpy(),
                                image_key=int(data['image_keys'][key].numpy()[0]),
                                bbox_covar=data['predicted_covar_mats'][key].numpy(),
                                pose=orb_pose,
